...
## Решение
Добавлен CreateView для создания нового объекта Robot. <br>
Запрос:
```http
...
{"model":"48","version":"on","created":"2022-12-31 23:59:59"}
```
...
```
## Решение
- Добавлен ListView для формирования отчёта в Excel. <br>
- Добавлен кастомный пагинатор для удобного разделения отчёта по страницам.
Его можно использовать в случае масштабирования и для других представлений
(не только для Excel). <br>
- Файл генерируется и скачивается по ссылке:
http://localhost:8000/report/week/last
...
## Решение
- Добавлен url где можно оформить заказ: http://localhost:8000/orders/add/
- Добавлен сигнал для отправки сообщения о готовности робота.
- Механизм уведомления клиента сделан как можно гибче, чтобы можно было изменять логику очереди клиентов или способа уведомления.